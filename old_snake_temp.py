    ...
    try:
        ...
        while True: # Main game loop
            ...
            This ensures that objects created *during* this step will NOT
                execute step events
            """
            current_instances = tuple(all_instances)
            ...
            for ev in events:
                if is_quit_event(ev):
                    terminate()
                    # todo eventually take this out and make you implement it yourself?
                for inst in current_instances:
...
def is_quit_event(ev):
    return (ev.type == QUIT or (ev.type == KEYUP and ev.key == K_ESCAPE))
